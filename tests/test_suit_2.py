...
class TestSuit1:
    ...
    @allure.description('Создаем нового пользователя, отправляем запрос c неверным значением параметра {parameter} созданного пользователя на ручку (POST api/auth/login) авторизации пользователя, проверяем код и тело ответа')
    @allure.story("Тестовый сценарий № 2")
    @allure.link(WEBPAGE, name='Учебный сервис «Stellar Burgers» (стенд)')
    def test_not_allowed_authorization_user_with_wrong_values(self, random_user, parameter):
        ...
        response = RequestTools.try_user_authorization(user=random_user)
        RequestTools.check_response_have_content(actually_value=response,results=results['NOT_ALLOWED_AUTHORIZATION_USER_WITH_WRONG_VALUES'])
    ...
    @allure.description('Создаем нового пользователя, отправляем запрос без указания параметра {parameter} созданного пользователя на ручку (POST api/auth/login) авторизации пользователя, проверяем код и тело ответа')
    @allure.story("Тестовый сценарий № 2")
    @allure.link(WEBPAGE, name='Учебный сервис «Stellar Burgers» (стенд)')
